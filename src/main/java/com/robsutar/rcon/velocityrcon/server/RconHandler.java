...
import com.robsutar.rcon.VelocityRcon;
import com.robsutar.rcon.velocityrcon.commandsource.IRconCommandSource;
    ...
    private void handleCommand(ChannelHandlerContext ctx, String payload, int requestId) {
        ...
        if (payload.equalsIgnoreCase("end") || payload.equalsIgnoreCase("stop")) {
            ...
            try {
                ...
                Bukkit.getGlobalRegionScheduler().run(VelocityRcon.getInstance(),(task)->{
                    rconServer.getServer().dispatchCommand(rconServer.getServer().getConsoleSender(),payload);
                });
                success = true;
